package lesson1;

import java.util.HashSet;
import java.util.LinkedList;
import java.util.List;
import java.util.Set;

public class Finite_Automata_Segment {
    private Set<String> keyWords;
    private int index;
    private StringBuilder sb;
    private LinkedList<String> symbolList;
    private String toTrans;

    public static void main(String[] args)  {
        Finite_Automata_Segment segment=new Finite_Automata_Segment();
        String[] ag={
                "public static void main(String[] args)",
                "int i=ter-15*(240.4+ci)",
                "int i=ter-15*( -240.4.4+ci)",
                "123.+.123",
                "System.out.println(a+123)"
        };
        for (int i = 0; i < ag.length; i++) {
            try {
                for (String str : segment.stateMachine(ag[i]) ) {
                    System.out.println(str);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            System.out.println("--------------------------------------");
        }

    }

    public Finite_Automata_Segment(){
        keyWords = new HashSet<>();
        setKeyWords();
    }

    private void init(String str){
        symbolList = new LinkedList<>();
        sb=new StringBuilder();
        toTrans=str;
        index =0;
    }

    private List<String> stateMachine(String str) throws Exception {
        init(str);
        while (index <str.length())
            node_0();
        return symbolList;
    }

    private void node_0() throws Exception {
        for(; index <toTrans.length(); index++){
            switch (getCharState(toTrans.charAt(index))){
                case 1:
                    sb.append(toTrans.charAt(index));
                    index++;
                    node_1();
                    return;
                case 2:
                    sb.append(toTrans.charAt(index));
                    index++;
                    node_2();
                    return;
                case 3:case 4:
                    sb.append(toTrans.charAt(index));
                    index++;
                    node_3();
                    return;
                case -1:
                    throw new Exception();
                default:
                    break;
            }
        }
    }
    private void node_1() throws Exception {
        upper:for(; index <toTrans.length(); index++) {
            switch (getCharState(toTrans.charAt(index))){
                case 1: case 2:
                    sb.append(toTrans.charAt(index));
                    break;
                case -1:
                    throw new Exception();
                default:
                    break upper;
            }
        }
        if(keyWords.contains(sb.toString())){
            node_end("keyword");
        }else {
            node_end("ident");
        }
    }
    private void node_2() throws Exception {
        upper:for(; index <toTrans.length(); index++) {
            switch (getCharState(toTrans.charAt(index))){
                case 4:
                    sb.append(toTrans.charAt(index));
                    index++;
                    node_2_1();
                    return;
                case 2:
                    sb.append(toTrans.charAt(index));
                    break;
                case -1:
                    throw new Exception();
                default:
                    break upper;
            }
        }
        node_end("int");
    }
    private void node_2_1() throws Exception {
        if(getCharState(toTrans.charAt(index))==2) {
            sb.append(toTrans.charAt(index));
            index++;
            node_2_2();
        }
        else
            throw new Exception();
    }
    private void node_2_2() throws Exception {
        upper:for(; index <toTrans.length(); index++) {
            switch (getCharState(toTrans.charAt(index))){
                case 2:
                    sb.append(toTrans.charAt(index));
                    break;
                case -1:
                    throw new Exception();
                default:
                    break upper;
            }
        }
        node_end("double");
    }
    private void node_3() throws Exception {
        upper:for(; index <toTrans.length(); index++) {
            switch (getCharState(toTrans.charAt(index))){
                case 3: case 4:
                    sb.append(toTrans.charAt(index));
                    break;
                case -1:
                    throw new Exception();
                default:
                    break upper;
            }
        }
        node_end("mark");
    }
    private void node_end(String type) {
        symbolList.add(sb.toString());
        sb=new StringBuilder();

        System.out.println(symbolList.getLast()+" is "+type);
    }

    private int getCharState(char c){
        switch (c){
            case ' ':case ';':case '\n':case '\r':
                return 0;
            case 'a':case 'b':case 'c':case 'd':case 'e':
            case 'f':case 'g':case 'h':case 'i':case 'j':
            case 'k':case 'l':case 'm':case 'n':case 'o':
            case 'p':case 'q':case 'r':case 's':case 't':
            case 'u':case 'v':case 'w':case 'x':case 'y':
            case 'z':case 'A':case 'B':case 'C':case 'D':
            case 'E':case 'F':case 'G':case 'H':case 'I':
            case 'J':case 'K':case 'L':case 'M':case 'N':
            case 'O':case 'P':case 'Q':case 'R':case 'S':
            case 'T':case 'U':case 'V':case 'W':case 'X':
            case 'Y':case 'Z': case '_':
                return 1;
            case '0':case '1':case '2':case '3':case '4':
            case '5':case '6':case '7':case '8':case '9':
                return 2;
            case '=':case '<':case '>':case '!':case '%':
            case '[':case ']':case '{':case '}':case '\'':
            case '\"':case '?':case ':':case ',':case '/':
            case '\\':case '~':case '*':case '&':case '|':
            case '(':case ')':case '+':case '-':
                return 3;
            case '.':
                return 4;
        }

        return -1;
    }

    public void setKeyWords() {
        keyWords.add("int");
        keyWords.add("double");
        keyWords.add("float");
        keyWords.add("String");
        keyWords.add("boolean");
        keyWords.add("var");
        keyWords.add("Object");
        keyWords.add("void");
        keyWords.add("public");
        keyWords.add("private");
        keyWords.add("class");
        keyWords.add("import");
        keyWords.add("include");
        keyWords.add("return");
        keyWords.add("null");
        keyWords.add("sqrt");
        keyWords.add("abs");
        keyWords.add("sin");
        keyWords.add("cos");
        keyWords.add("pow");
        keyWords.add("max");
        keyWords.add("min");
        keyWords.add("static");
//        keyWords.add("");
    }
}
