...
public class GrammerFileReader {
     ...
     * @return
     */
    public static Map<String,List<String>> makeConbinationGrammer(Collection<String> lines){
        Map<String,List<String>> productions=new HashMap<>();
        for(String rule:lines){
            int splitINdex=rule.indexOf(':');
            String prodStr=rule.substring(0,splitINdex).strip(),prod=rule.substring(splitINdex+1).strip();

            if (!productions.containsKey(prodStr)) {
                ArrayList<String> list=new ArrayList<>();
                productions.put(prodStr,list);
            }
            productions.get(prodStr).add(prod);
        }
        return productions;
    }
}